...
fn play_tts_inner(text: String, app: AppHandle) {
    ...
    run_tts(&app, "main", text, voice, speed);
}
...
fn preview_tts_inner(voice: String, speed: f64, app: AppHandle) {
    ...
    run_tts(&app, "settings", text, voice, speed);
}

/// Play `text` in `voice` at `speed`, notifying `window_label` of progress.
/// Shared by playing a translation result and previewing a voice in Settings.
fn run_tts(app: &AppHandle, window_label: &str, text: String, voice: String, speed: f64) {
    start_tts(app, window_label);
    ...
    let api_key = match app.state::<AppState>().config_store.api_key() {
        Ok(key) => key,
        Err(message) => {
            let _ = app.emit_to(window_label, "tts-error", message);
            emit_tts(app, "tts-done");
            return;
        }
    };
