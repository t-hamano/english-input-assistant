...
#### アップデート後のアクセシビリティの再登録

現在の配布版では、アプリをアップデートするたびにアクセシビリティの一覧から削除し、追加し直す必要があります。スイッチがオンでも、更新後のアプリに権限が引き継がれず、エラーになることがあります。

1. メニューバーのアプリアイコンからアプリを完全に終了します。
2. **システム設定 → プライバシーとセキュリティ → アクセシビリティ** を開きます。
3. **English Input Assistant** を選択し、「−」で削除します。
4. 「＋」から `/Applications/English Input Assistant.app` を選択して **開く** を押し、スイッチをオンにします。
5. アプリを起動し直します。
